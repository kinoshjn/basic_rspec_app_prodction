...
# 1.1. ログイン前  ページ遷移確認
  it 'ログイン前はタスク新規登録ページにアクセスできない' do
    visit new_task_path  # 新規登録ページへアクセス
    expect(current_path).to eq login_path
  end
  it 'タスクの編集ページにアクセス => アクセス失敗' do
    visit edit_task_path  # 新規登録ページへアクセス
    expect(current_path).to eq login_path
  end
  ...
  it 'タスク一覧ページにアクセスならタスク一覧が表示される' do
    ...
    expect(current_path).to eq index_task_path
  end
...
# 1.2.1. タスクの新規登録 
  ...
  it '登録済みのタイトルを入力するなら編集失敗' do
    user = create(:user)
    create(:task, title: '既存タスク')
    task = create(:task, title: 'テストタスク')
    visit login_path
    fill_in 'Email', with: user.email
    fill_in 'Password', with: 'password'
    click_button 'Login'
